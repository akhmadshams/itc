...
class IndexView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        staff_count = Staff.objects.filter().count()
        invoice_count = Invoice.objects.filter().count()
        result_count = Result.objects.filter().count()
        class_count = StudentClass.objects.filter().count()
...
class SiteConfigView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset.is_valid():
            formset.save()
            messages.success(request, "Konfiguratsiyalar muvaffaqiyatli yangilandi")
